...
func (s *DayStore) Create(date time.Time, calories int, userID uint32) (*domain.Day, error) {
	...
	updatedAt := time.Now()
	return scanDay(s.db.QueryRow(query, date, calories, userID, updatedAt))
}

func (s *DayStore) FindByID(id uint32) (*domain.Day, error) {
	query := `SELECT "id", "date", "calories", "userId", "createdAt", "updatedAt" FROM "Day" WHERE "id" = $1`
	return scanDay(s.db.QueryRow(query, id))
}
